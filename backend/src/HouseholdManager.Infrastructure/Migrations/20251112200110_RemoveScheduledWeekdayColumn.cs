...
namespace HouseholdManager.Infrastructure.Migrations
{
    ...
    public partial class RemoveScheduledWeekdayColumn : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // Detect database provider and execute appropriate SQL
            if (migrationBuilder.ActiveProvider == "Npgsql.EntityFrameworkCore.PostgreSQL")
            {
                ...
                migrationBuilder.Sql(@"
                    DO $$
                    BEGIN
                        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'HouseholdTasks') THEN
            ...
            else
            {
                // SQL Server syntax
                migrationBuilder.Sql(@"
                    IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'HouseholdTasks')
                    BEGIN
                        UPDATE HouseholdTasks
                        SET RecurrenceRule =
                            CASE ScheduledWeekday
